...
- **AI Trading Signal**: An automated quantitative recommendation engine (STRONG BUY, BUY, HOLD, SELL) that combines NLP sentiment scores with short-term technical price momentum.
...
4. Below the chart, the AI will process the latest news headlines and display the sentiment breakdown (Positive, Neutral, Negative).
5. The **AI Trading Signal** will evaluate the overall sentiment ratio alongside the stock's 5-day momentum to output an actionable trading recommendation (e.g., STRONG BUY 🚀).
6. Explore the interactive table at the bottom to read the exact news articles that influenced the model.
